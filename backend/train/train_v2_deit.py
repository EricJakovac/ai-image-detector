...
def train_deit_v2_1k():
    # --- DEVICE ---
    device = torch.device("cpu")
    print(f"🔥 DETEKTIRANA SNAGA (DeiT V2): CPU")

    # --- PARAMETRI ---
    MODEL_NAME = "DeiT_PT16_V2"
    IMG_SIZE = 224
    BATCH_SIZE = 8
    ...
    V1_MODEL_PATH = os.path.join(
        ...
        "models",
        "v1_models",
        "DeiT_PT16_IMG224_B8_LR1e-5_Acc95.02_E4.pth",
    )
    ...
    # --- Freeze except head + last blocks ---
    params_to_update = []
    for name, param in model.named_parameters():
        if any(
            keyword in name
            for keyword in ["head", "classifier", "norm", "blocks.10", "blocks.11"]
        ):
    ...
    print(
        f"❄️ Zamrznuto slojeva: {len(list(model.named_parameters())) - len(params_to_update)}/{len(list(model.named_parameters()))}"
    )
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            # ✅ Batch-level ispis svakih 50 batch-eva
            if (batch_idx + 1) % 50 == 0:
    ...
    print(f"🏁 DeiT V2 treniranje kompletirano! Najbolja točnost: {best_acc:.2f}%")
    return best_acc
