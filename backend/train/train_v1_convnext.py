# backend/train/train_v1_convnext.py
import os
import sys
import torch
...
def train_convnext_windows_native():
    # --- POSTAVA UREĐAJA (AMD GPU) ---
    if torch_directml.is_available():
        device = torch_directml.device()
        print(f"🔥 DETEKTIRANA SNAGA (ConvNeXt): {torch_directml.device_name(0)}")
    else:
    ...
    IMG_SIZE = 224
    BATCH_SIZE = 16  # ConvNeXt može veći batch
    ACCUMULATION_STEPS = 4  # Efektivni batch = 64
    EPOCHS = 5
    LEARNING_RATE = 2e-4  # Normalan LR za ConvNeXt

    # --- PAMETNO UPRAVLJANJE PUTANJAMA ---
    ...
    print(f"------------------------------------------")
    print(f"🚀 POKRETANJE ConvNeXt TRENINGA")
    ...
    print(f"🏗️ Učitavam ConvNeXt Tiny pre-trained težine...")
    # ConvNeXt Tiny - moderna alternativa Swinu
    model = timm.create_model("convnext_tiny", pretrained=True, num_classes=2).to(
        device
    )
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % 100 == 0:
                print(
                    f"⚡ [ConvNeXt Train] Batch: {batch_idx}/{len(train_loader)} | Loss: {loss.item() * ACCUMULATION_STEPS:.4f}"
                )
        ...
        file_name = f"{MODEL_NAME}_IMG{IMG_SIZE}_B{BATCH_SIZE}_LR2e-4_Acc{acc_str}_E{epoch+1}.pth"
    ...
    print(f"\n🏁 ConvNeXt TRENING ZAVRŠEN!")


if __name__ == "__main__":
    train_convnext_windows_native()
